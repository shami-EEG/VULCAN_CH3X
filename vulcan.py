...
if vulcan_cfg.const_mix_with_EQ:
    for sp in vulcan_cfg.const_mix_with_EQ.keys():
        data_var.ymix[:,species.index(sp)] = vulcan_cfg.const_mix_with_EQ[sp]
        data_var.y[:,species.index(sp)] = vulcan_cfg.const_mix_with_EQ[sp] *data_atm.n_0
# modifed for the Hycean DMS run
...
output.plot_evo(data_var, data_atm, plot_j=10, dn=1, abun_min=1e-10)